...
class DataHandler:
    ...
    def register_dataset(self, dataset: Dataset):
        ...
        if len(self.dataset_ids) < 1:
            ...
            dataset_id = min(
                set(range(largest_id + 2)) - set(self.dataset_ids))
    ...
    def _getdataset_id_from_id_or_name(self, identifier: Union[str, int]):
        ...
        try:
            ...
            if identifier not in self.dataset_names:
                ...
                dataset_id = self.dataset_ids[self.dataset_names.index(
                    identifier)]
...
class Dataset:
    ...
    def __init__(self,
        ...
        if not isinstance(data_handler, DataHandler):
            raise ValueError(f'`data_handler` must be of type DataHandler,\
        ...
        for instring in d:
            try:
                ...
                if not self.raise_smiles_errors:
                    logging.info(
                        f'"{instring}" could not be connonicalized, skipping')
                    pass
                else:
                    raise ValueError(
                        f'"{instring}" could not be connonicalized')
